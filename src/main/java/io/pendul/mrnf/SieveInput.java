...
public class SieveInput {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(SieveInput.class);
    /**
     ...
     *
     * @param N    is the integer to factor.
     * @param hdfs is the file file system.
     * @param file is the name of the input file.
     */
    public static void writeFullSieveInterval(BigInteger N, FileSystem hdfs, Path file) throws IOException {
        if (hdfs.exists(file)) {
            hdfs.delete(file, true);
        }
        LOGGER.info("Generating input file: {}", file);
        ...
        int intervalSize = QuadricSieveDriver.MAP_INPUT_INTERVAL_SIZE;
