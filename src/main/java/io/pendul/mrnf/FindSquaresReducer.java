...
public class FindSquaresReducer extends Reducer<Text, Text, Text, Text> {
    ...
    protected void setup(Context context) throws IOException {
        ...
        N = new BigInteger(context.getConfiguration().get(
                        QuadricSieveDriver.INTEGER_TO_FACTOR_NAME));
        try {
            factorBase = FactorBaseArray.fromString(context.getConfiguration().get(
                            QuadricSieveDriver.FACTOR_BASE_NAME));
    ...
    public void reduce(Text key, Iterable<Text> values, Context context) {
        ...
        if (factor == null) {
            LOGGER.severe("Unable to factor N after: " + String.valueOf(MAX_FIND_SQUARE_ATTEMPTS) + " attempts.");
            return;
        }
        ...
        try {
            ...
            reducer_unable_to_output.increment(1);
            LOGGER.severe("Unable to write the reduce output. InterrptedException: " + e);
            return;
        }
    }
}
